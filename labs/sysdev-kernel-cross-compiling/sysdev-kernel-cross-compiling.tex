...
Extract \code{kernel-angstrom.tar.gz} in the current folder.

\begin{verbatim}
tar -xf kernel-angstrom.tar.gz
\end{verbatim}
...
There is a config provided, the \code{defconfig} file in the \code{~/labs/kernel} folder.
...
Plug the BeagleBone board on your computer. Start Picocom on \code{/dev/ttyUSB1}
